...
/*
#define exc_active_(fname, ...) \
    exc_active_type->exc_##fname(exc_active_value __VA_OPT__(,) __VA_ARGS__)
#define exc_active_message() \
    exc_active_type ? exc_active_(get_message) : NULL

#define exc_handle(stmt) \
    stmt; if (exc_active_type)
#define exc_assert(stmt, error_return) \
    exc_handle(stmt) return
#define exc_assert_(stmt, error_return) \
    exc_handle(stmt) return error_return
*/

/* e.g.:
 *  result = exc_handle(some_operation()) {
 *      if (exc_is(exc_IOError)) {
 *          // Handle error
 *          exc_clear();
 *      } else {
 *          // Must manually abort!
...
inline void exc_raise(exc_type_t exc_type, exc_value_t exc_value)
{
	...
	if (exc_is(exc_CallerError)) {
		// print message, force exit
		// maybe use a configurable handler
	}
}
...
inline void exc_clear()
{
	if (exc_active_type) {
		// exc_active_(clear);
		for (exc_type_t type = exc_active_type; type != NULL; type = type->exc_parent) {
